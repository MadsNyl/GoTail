...
sqlite3 /app/db.sqlite "SELECT COUNT(*) FROM log;"

# Count logs older than 30 days
old_logs_count=$(sqlite3 /app/db.sqlite "SELECT COUNT(*) FROM log WHERE timestamp < datetime('now', '-30 days');")
echo "[cleanup.sh] Logs older than 30 days: $old_logs_count"

# Delete logs older than 30 days where severity_text is INFO
sqlite3 /app/db.sqlite <<EOF
DELETE FROM log
